...
int main(int argc, char **argv)
{
   ...
   double *pin_re  = (double *) _mm_malloc(plqcd_g.VOLUME*12*sizeof(double), 16);
   double *pin_im  = (double *) _mm_malloc(plqcd_g.VOLUME*12*sizeof(double), 16);
   double *pout_re = (double *) _mm_malloc(plqcd_g.VOLUME*12*sizeof(double), 16);
   double *pout_im = (double *) _mm_malloc(plqcd_g.VOLUME*12*sizeof(double), 16);
   double *u_re    = (double *) _mm_malloc(plqcd_g.VOLUME*36*sizeof(double), 16);
   double *u_im    = (double *) _mm_malloc(plqcd_g.VOLUME*36*sizeof(double), 16);
   ...
   int  matvecs;
   double myval=0;  //use this to avoid code optimization by the compiler

   #ifdef SSE3_INTRIN
   matvecs=0;
   total=0.0;
   mytotal =0.0;
   t1=plqcd_hopping_matrix_eo_sse3_intrin_nohalfspinor(pout_re,pout_im,u_re,u_im,pin_re,pin_im);
   ...
   if(proc_id==0)
   {
     fprintf(ofp,"sse3 with intrinsics and split layout no-halfspinor:\n");
     fprintf(ofp,"------------------------------------------\n");
